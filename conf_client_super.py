...
class VideoStreamTrack(VideoStreamTrack):
    ...
    async def recv(self):
        ...
        try:
            ...
            global screen,camare,voice,play,brightness  # 明确声明这是全局变量

            # print(f"camare:{camare}")
            if camare and not screen:
                # camare 不需要BGR转换
                # frame_bgr = np.array(capture_camera().set(cv2.CAP_PROP_BRIGHTNESS, brightness))
...
class ConferenceClientGUI:
    ...
    def create_widgets(self):
        self.username_label = tk.Label(self.root, text="Username:", bg="pink")
        self.username_label.grid(row=0, column=0, padx=10, pady=10)
        self.username_entry = tk.Entry(self.root)
        self.username_entry.grid(row=0, column=1, padx=10, pady=10)

        self.conference_id_label = tk.Label(self.root, text="Conference ID:", bg="pink")
        ...
        self.quit_button = tk.Button(self.root, text="Quit Conference", command=self.quit_conference_wrapper, bg="red",
                                     fg="white")
        ...
        self.switch_audio_button = tk.Button(self.root, text="Switch Audio: Off", command=lambda: self.switch_data('voice'),
                                             bg="orange", fg="white")
        self.switch_audio_button.grid(row=4, column=1, padx=10, pady=10)

        # image_paths = ["lb.png", "xj.png", "pm.png"]
        # coordinates = [(750, 260), (750, 340), (250, 340)]
        # for path,(x,y) in zip(image_paths,coordinates):
        #     self.add_image(path, x, y)

        self.switch_screen_button = tk.Button(self.root, text="Switch Screen: Off",
                                              command=lambda: self.switch_data('screen'), bg="orange", fg="white")
        self.switch_screen_button.grid(row=5, column=0, padx=10, pady=10)

        self.switch_camera_button = tk.Button(self.root, text="Switch Camera: Off",
                                              command=lambda: self.switch_data('camare'), bg="orange", fg="white")
        self.switch_camera_button.grid(row=5, column=1, padx=10, pady=10)

        self.message_label = tk.Label(self.root, text="Message to send:", bg="pink")
        self.message_label.grid(row=6, column=0, padx=10, pady=10)

        self.message_entry = tk.Entry(self.root)
        self.message_entry.grid(row=6, column=1, padx=10, pady=10)

        self.send_button = tk.Button(self.root, text="Send Message", command=self.send_message_wrapper, bg="purple",
                                     fg="white")
        self.send_button.grid(row=7, column=0, columnspan=2, padx=10, pady=10)

        self.response_label = tk.Label(self.root, text="Server Response:", bg="pink")
        ...
        self.volume_label = tk.Label(self.root, text="Volume:", bg="pink")
        self.volume_label.grid(row=9, column=0, padx=10, pady=10)
        ...
        self.volume_scale.grid(row=9, column=1, padx=10, pady=10)

        self.volume_label = tk.Label(self.root, text="Brightness:", bg="pink")
        self.volume_label.grid(row=10, column=0, padx=10, pady=10)
        self.brightness_scale = tk.Scale(self.root, from_=0, to=100, orient=tk.HORIZONTAL,command=self.adjust_brightness)
        self.brightness_scale.set(50)  # 设置默认亮度值
        self.brightness_scale.grid(row=10, column=1,padx=10, pady=10)
    ...
    def create_background(self):
        ...
        # 如果你还想创建一个Frame作为背景，可以这样做：
        self.background = tk.Frame(self.root, bg="pink")
        self.background.place(relwidth=1, relheight=1)  # 使Frame充满整个窗口

        # 如果你不需要在背景上放置任何其他控件，就不需要创建Label
        # 如果需要在背景上放置控件，确保它们的bg属性设置为"pink"或者透明，以显示背景颜色


    def add_image(self, image_path, x, y):
        # 加载图片文件
        image = Image.open(image_path)  # 传入图片路径
        ...
        # 使用place布局管理器来设置Label的确切位置
        # 注意：这里的x和y是你希望图片左上角开始的坐标
        image_label.place(x=x,y=y)  # 例如，这里将图片放置在距离窗口左边缘10像素，上边缘50像素的位置
    ...
    def update_button_text(self):
        self.switch_audio_button.config(text=f"Switch Audio: {'On' if voice else 'Off'}")
        self.switch_screen_button.config(text=f"Switch Screen: {'On' if screen else 'Off'}")
        self.switch_camera_button.config(text=f"Switch Camera: {'On' if camare else 'Off'}")
