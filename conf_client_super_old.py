...
class ConferenceClient:
    ...
    async def create_conference(self):
        ...
        if self.on_meeting:
            ...
            return
        if self.Main_reader is None and self.Main_writer is None:
            self.Main_reader, self.Main_writer = await asyncio.open_connection(*self.server_addr)

        # Send create request with username
        create_cmd = f"create {self.username}"
        self.Main_writer.write(create_cmd.encode())
        await self.Main_writer.drain()
        data = await self.Main_reader.read(100)
        ...
        if "created" in response:
            ...
            self.Main_writer.write(join_cmd.encode())
            await self.Main_writer.drain()
            data = await self.Main_reader.read(100)
            response = data.decode()
            print(f"Received: {response}")
    ...
    async def quit_conference(self):
        ...
        try:
            if self.on_meeting and self.username:
                ...
                self.Main_writer.write(quit_cmd.encode())
                await self.Main_writer.drain()
                data = await self.Main_reader.read(100)
    ...
    async def cancel_conference(self):
        ...
        self.Main_writer.write(cancel_cmd.encode())
        await self.Main_writer.drain()
        data = await self.Main_reader.read(100)
        response = data.decode()
        print(f"Received: {response}")
    ...
    async def start_conference(self, ip, port):
        ...
        def on_track(track):
            ...
            if track.kind == "video":
                ...
                self.video_task = asyncio.create_task(self.handle_video_track())
            elif track.kind == "audio":
                self.received_audio_track = track
                self.audio_task = asyncio.create_task(self.handle_audio_track())
